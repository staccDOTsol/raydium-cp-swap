...
/// Verifies `stake_pool` is the stake pool whose pool mint is `mint`, owned by the ruleset's program,
/// and returns the exchange rate (1e9 == 1 SOL per pool token).
pub fn stake_pool_rate(ruleset: &Ruleset, mint: &Pubkey, stake_pool: &AccountInfo) -> Result<u64> {
    ...
    u64::try_from(u128::from(total_lamports) * u128::from(RATE_ONE) / u128::from(supply)).map_err(|_| ErrorCode::MathOverflow.into())
}
...
pub fn validate_ruleset_params(kind: u8, flags: u8, program_id: &Pubkey) -> Result<()> {
    match RuleKind::from_u8(kind).ok_or(ErrorCode::InvalidRuleKind)? {
        RuleKind::PumpFunLaunch => {
            require_keys_neq!(*program_id, Pubkey::default(), ErrorCode::InvalidInput);
            require!(flags & !(FLAG_ALLOW_MAYHEM | FLAG_REQUIRE_COMPLETE) == 0, ErrorCode::InvalidInput);
        }
        RuleKind::Lst | RuleKind::LaunchpadDbc => {
            require_keys_neq!(*program_id, Pubkey::default(), ErrorCode::InvalidInput);
            require_eq!(flags, 0, ErrorCode::InvalidInput);
        }
        _ => require_eq!(flags, 0, ErrorCode::InvalidInput),
    }
    Ok(())
}
