...
/// Adds a collection member to a collection pool: creates its vault under the pool authority.
...
pub fn add_pool_member(ctx: Context<AddPoolMember>) -> Result<()> {
    ...
    require!(pm.find(&mint).is_none(), ErrorCode::PoolMemberExists);
    {
        ...
        require!(mint != pool.token_0_mint && mint != pool.token_1_mint, ErrorCode::PoolMemberExists);
    }
    ...
    create_token_account(
        ...
        &[
            MEMBER_VAULT_SEED.as_bytes(),
            pool_key.as_ref(),
            mint.as_ref(),
            &[ctx.bumps.member_vault][..],
        ],
    )?;
