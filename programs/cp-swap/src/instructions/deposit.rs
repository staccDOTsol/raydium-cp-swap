...
use anchor_spl::token::Mint as m2;
...
use anchor_spl::token::InitializeMint2;
use anchor_spl::token::Token;
use anchor_spl::token_interface::{Mint, Token2022, TokenAccount};
use mpl_token_metadata::types::DataV2;
use spl_math::uint::U256;
use spl_memo::solana_program::program::invoke;
use spl_memo::solana_program::program::invoke_signed;
use spl_memo::solana_program::program_memory::sol_memcmp;
use spl_memo::solana_program::pubkey::PUBKEY_BYTES;
...
pub fn deposit<'info>(
    ...
    if pool_state.token_0_vault_safu == Pubkey::default() || pool_state.token_1_vault_safu == Pubkey::default() {
...
let existing_metadata = MetadataAccount::try_deserialize_unchecked(&mut &metadata_account.data.borrow_mut()[..])?;
...
pool_state.safu_lp_supply = pool_state.safu_lp_supply.checked_add(liquidity.try_into().unwrap()).unwrap();
    ...
    let token_0_vault_safu_amount = spl_token_2022::extension::StateWithExtensions::<spl_token_2022::state::Account>::unpack(
        *token_0_vault_safu.to_account_info().try_borrow_data()?
    )?.base.amount;
    let token_1_vault_safu_amount = spl_token_2022::extension::StateWithExtensions::<spl_token_2022::state::Account>::unpack(
        *token_1_vault_safu.to_account_info().try_borrow_data()?
    )?.base.amount;
    let (total_token_0_amount, total_token_1_amount) = pool_state.vault_amount_without_fee(
        token_0_vault_safu_amount,
        token_1_vault_safu_amount,
    );
    ...
    let (safu_transfer_token_0_amount, safu_transfer_token_0_fee) = {
        let transfer_fee = get_transfer_inverse_fee(&ctx.accounts.vault_0_mint.to_account_info(), safu_token_0_amount)?;
        (
    ...
    let (safu_transfer_token_1_amount, safu_transfer_token_1_fee) = {
        let transfer_fee = get_transfer_inverse_fee(&ctx.accounts.vault_1_mint.to_account_info(), safu_token_1_amount)?;
        (
    ...
    pool_state.safu_lp_supply = pool_state.safu_lp_supply.checked_add(safu_lp_token_amount.try_into().unwrap()).unwrap();
