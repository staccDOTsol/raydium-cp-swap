...
impl AMM {
    ...
    pub fn get_buy_price(&self, tokens: u128) -> Option<u128> {
        ...
        let product_of_reserves = self.virtual_liquidity.checked_mul(self.virtual_token_reserves)?;
        let new_virtual_token_reserves = self.virtual_token_reserves.checked_sub(tokens)?;
        let new_virtual_liquidity = product_of_reserves.checked_div(new_virtual_token_reserves)?.checked_add(1)?;
        let amount_needed = new_virtual_liquidity.checked_sub(self.virtual_liquidity)?;

        Some(amount_needed)
    }

    pub fn apply_buy(&mut self, real_liquidity: u128) -> Option<u128> {
        ...
        // self.virtual_liquidity = self.virtual_liquidity.checked_add(sol_amount)?;
        ...
        // })
        let new_supply = self.virtual_liquidity.checked_div(real_liquidity).unwrap();
        Some(new_supply)
    }

    pub fn apply_sell(&mut self, virtual_amount: u128) -> Option<u128> {
        let new_supply = self.virtual_liquidity.checked_div(virtual_amount).unwrap();
        Some(new_supply)
    }
    ...
    pub fn get_sell_price(&self, tokens: u128) -> Option<u128> {
        ...
        let sol_received = (self.virtual_liquidity.checked_mul(token_sell_proportion)?).checked_div(scaling_factor)?;
...
impl fmt::Display for AMM {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "AMM {{ virtual_liquidity: {}, virtual_token_reserves: {}, real_sol_reserves: {}, real_token_reserves: {}, initial_virtual_token_reserves: {} }}",
            self.virtual_liquidity, self.virtual_token_reserves, self.real_sol_reserves, self.real_token_reserves, self.initial_virtual_token_reserves
        )
    }
}
...
mod tests {
    ...
    //     assert!(amm.virtual_liquidity >= DEFAULT_VIRTUAL_SOL_RESERVE);
