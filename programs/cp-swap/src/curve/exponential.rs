...
mod tests {
    ...
    fn test_get_sell_price() {
        ...
        // Check if selling more than available tokens returns None
        assert!(amm.get_sell_price(DEFAULT_TOKEN_RESERVES * 2).is_none());
    }
    ...
    fn test_get_buy_price() {
        ...
        // Check if buying more than available tokens returns None
        assert!(amm.get_buy_price(30_000_000_000 * 2).is_some());
    }

    #[test]
    fn test_get_buy_price_edge_cases() {
        let amm = AMM::new(
            DEFAULT_VIRTUAL_SOL_RESERVE,
            DEFUALT_VIRTUAL_TOKEN_RESERVE,
            0,
        ...
        let small_amount = 1;
        let small_price = amm.get_buy_price(small_amount);
        assert!(small_price.is_some());
        assert!(small_price.unwrap() > 0);

        // Test with amount just below the available tokens
        let large_amount = DEFUALT_VIRTUAL_TOKEN_RESERVE - 1;
        let large_price = amm.get_buy_price(large_amount);
        assert!(large_price.is_some());
        assert!(large_price.unwrap() > 0);

        // Test with amount exceeding available tokens
        let exceed_amount = DEFUALT_VIRTUAL_TOKEN_RESERVE + 1;
        assert!(amm.get_buy_price(exceed_amount).is_none());

        // Test with maximum possible amount
        let u128_max = u128::MAX;
        assert!(amm.get_buy_price(u128_max).is_none());
    }
}